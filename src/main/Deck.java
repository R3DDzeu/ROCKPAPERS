...
static int[] ofSpades= new int[13];
    ...
    public static int takeACard(){
       ...
       switch (deck){
           case 0 -> { int pos=random.nextInt(ofSpades.length);
               if(pos<=10 && pos!=0){
                   card = ofSpades[pos];
                   System.out.println(card + " of Spades");
               } else if(pos==0){card = ofSpades[pos];
                   System.out.println("Ace of Spades");} else {card = 10;
                   switch (pos) {
           ...
           case 1 -> { int pos=random.nextInt(ofHearts.length);
               if(pos<=9 && pos!=0){
                   card = ofHearts[pos];
                   System.out.println(card + " of Hearts");
               } else if(pos==0){card = ofHearts[pos];
                   System.out.println("Ace of Hearts");} else {card = 10;
                   switch (pos) {
           ...
           case 2 -> { int pos=random.nextInt(ofClubs.length);
               if(pos<=9 && pos!=0){
                   card = ofClubs[pos];
                   System.out.println(card + " of Clubs");
               } else if(pos==0){card = ofSpades[pos];
                   System.out.println("Ace of Clubs");} else {card = 10;
                   switch (pos) {
           ...
           case 3 -> { int pos=random.nextInt(ofDiamonds.length);
               if(pos<=9 && pos!=0){
                   card = ofDiamonds[pos];
                   System.out.println(card + " of Diamonds");
               } else if(pos==0){card = ofDiamonds[pos];
                   System.out.println("Ace of Diamonds");} else {card = 10;
                   switch (pos) {
   ...
   static void imparteCarti(){
        ...
        if (pHand==21) {
            System.out.println("Blackjack!"); balance=balance+pariu;
        } else if (pHand!=21) { while(true) {
            System.out.println("Hit or stand?");
            Scanner scanner = new Scanner(System.in);
            hit = scanner.nextLine();
            if (hit.equalsIgnoreCase("hit")){ pHand= pHand+takeACard(); if (pHand>21) {
        ...
        while(dHand<17) { dHand = dHand + takeACard();}

        if (dHand>=17 || dHand<=21){ if(dHand>pHand) {
            System.out.println("You lose!"); balance = balance - pariu;
        } else if (dHand==pHand) {
